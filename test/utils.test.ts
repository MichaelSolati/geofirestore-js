...
describe('Utils Tests:', () => {
  ...
  describe('Encode geohash:', () => {
    it('encodeGeohash() encodes locations to geohashes given no precision', () => {
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(-90, -180))
      ).to.be.equal('0000000000'.slice(0, GEOHASH_PRECISION));
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(90, 180))
      ).to.be.equal('zzzzzzzzzzzz'.slice(0, GEOHASH_PRECISION));
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(-90, 180))
      ).to.be.equal('pbpbpbpbpbpb'.slice(0, GEOHASH_PRECISION));
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(90, -180))
      ).to.be.equal('bpbpbpbpbpbp'.slice(0, GEOHASH_PRECISION));
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(37.7853074, -122.4054274))
      ).to.be.equal('9q8yywe56gcf'.slice(0, GEOHASH_PRECISION));
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(38.98719, -77.250783))
      ).to.be.equal('dqcjf17sy6cp'.slice(0, GEOHASH_PRECISION));
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(29.3760648, 47.9818853))
      ).to.be.equal('tj4p5gerfzqu'.slice(0, GEOHASH_PRECISION));
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(78.216667, 15.55))
      ).to.be.equal('umghcygjj782'.slice(0, GEOHASH_PRECISION));
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(-54.933333, -67.616667))
      ).to.be.equal('4qpzmren1kwb'.slice(0, GEOHASH_PRECISION));
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(-54, -67))
      ).to.be.equal('4w2kg3s54y7h'.slice(0, GEOHASH_PRECISION));
    });

    it('encodeGeohash() encodes locations to geohashes given a custom precision', () => {
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(-90, -180), 6)
      ).to.be.equal('000000');
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(90, 180), 20)
      ).to.be.equal('zzzzzzzzzzzzzzzzzzzz');
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(-90, 180), 1)
      ).to.be.equal('p');
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(90, -180), 5)
      ).to.be.equal('bpbpb');
      expect(
        encodeGeohash(
          new firebase.firestore.GeoPoint(37.7853074, -122.4054274),
      ...
      ).to.be.equal('9q8yywe5');
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(38.98719, -77.250783), 18)
      ).to.be.equal('dqcjf17sy6cppp8vfn');
      expect(
        encodeGeohash(
          new firebase.firestore.GeoPoint(29.3760648, 47.9818853),
          12
        )
      ).to.be.equal('tj4p5gerfzqu');
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(78.216667, 15.55), 1)
      ).to.be.equal('u');
      expect(
        encodeGeohash(
          new firebase.firestore.GeoPoint(-54.933333, -67.616667),
      ...
      expect(
        encodeGeohash(new firebase.firestore.GeoPoint(-54, -67), 9)
      ).to.be.equal('4w2kg3s54');
    });
    ...
    it('encodeGeohash() does not throw errors given valid precision', () => {
      ...
      validPrecisions.forEach(validPrecision => {
        expect(() =>
          encodeGeohash(new firebase.firestore.GeoPoint(0, 0), validPrecision)
        ).not.to.throw();
      });
    });
  ...
  describe('Parameter validation:', () => {
    ...
    it('validateCriteria(criteria, true) does not throw errors given valid query criteria', () => {
      validQueryCriterias.forEach(validQueryCriteria => {
        if (
          typeof validQueryCriteria.center !== 'undefined' &&
          typeof validQueryCriteria.radius !== 'undefined'
        ) {
          expect(() =>
            validateQueryCriteria(validQueryCriteria, true)
          ).not.to.throw();
        }
      });
    });
    ...
    it('validateCriteria(criteria, true) throws errors given invalid query criteria', () => {
      invalidQueryCriterias.forEach(invalidQueryCriteria => {
        expect(() =>
          validateQueryCriteria(invalidQueryCriteria, true)
        ).to.throw();
      });
      expect(() =>
        validateQueryCriteria(
    ...
    it('validateCriteria(criteria) throws errors given invalid query criteria', () => {
      ...
      it('validateGeoFirestoreObject() throws errors given invalid GeoFirestoreObj', () => {
        invalidGeoFirestoreDocuments.forEach(invalidGeoFirestoreObject => {
          expect(() =>
            validateGeoDocument(invalidGeoFirestoreObject)
          ).to.throw();
        });
      });
      ...
      it('validateGeoFirestoreObject() returns false given invalid GeoFirestoreObj with flag enabled', () => {
        invalidGeoFirestoreDocuments.forEach(invalidGeoFirestoreObject => {
          expect(() =>
            validateGeoDocument(invalidGeoFirestoreObject, true)
          ).to.throw();
        });
      });
    });
